...
def fetch_jobs_from_adzuna(criteria):
    ...
    while True:
        logging.info(f"Récupération de la page {page} pour '{criteria['query']}'...")
        ...
        try:
            ...
            if page == 1:
                ...
                logging.info(f"Nombre total d'annonces disponibles : {total_count}")

            # Récupérer les résultats
            page_results = data.get("results", [])
            if not page_results:
                logging.info("Fin de la pagination : plus d'offres disponibles.")
            ...
            if len(page_results) < criteria["results_per_page"]:
                logging.info("Fin de la pagination (moins de résultats que demandés).")
                break
